...
var menu = [
  ...
  { 'type': 'separator' }, { 'type': 'bool', 'label': 'Ball quality is good', 'node': '/decision/isBallQualityGood' },
  { 'type': 'bool', 'label': 'Field quality is good', 'node': '/decision/isFieldQualityGood' },
  { 'type': 'bool', 'label': 'Is fallen', 'node': '/decision/isFallen' },
  { 'type': 'bool', 'label': 'Ball position is shared', 'node': '/decision/ballIsShared' },
  { 'type': 'bool', 'label': 'Am I handled ?', 'node': '/decision/handled' }, { 'type': 'separator' },
  { 'type': 'bool', 'readOnly': true, 'label': 'Should let play', 'node': '/decision/shouldLetPlay' },
  { 'type': 'bool', 'readOnly': true, 'label': 'I am captain', 'node': '/captain/IAmCaptain' }, { 'type': 'separator' },
  {
    'type': 'zone',
    'update': function(div) {
...
function redraw()
{
  ...
  if (kickController != 'none' && kickController != 'conflict')
  {
    ...
    {
      ctx.lineTo(
          ballX + Math.cos(kickControllerDir + alpha) * dist, ballY + Math.sin(kickControllerDir + alpha) * dist);
    }
...
function update()
{
  ...
  {
    var line = lines[k];
    var parts = line.split(' ').filter(function(x) {
      return x;
    });
    if (parts.length >= 2)
    {
